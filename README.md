...
| 器件 | 型号/规格 | 说明 |
|------|-----------|------|
| 锂电池 | 辰克 2800mAh（可以更小） | 小型号端子，紧凑安装 |
...
###1.理论知识学习指标
  - **江科协32单片机：** 关键知识点[io口上拉电阻控制led | 按键电容、软件消抖 | 通用寄存器、高级寄存器、基本寄存器定时器tim差异 | 驱动tb6612使用引脚控制电机
      --**模块原理图：** 上拉在原理图的绘制实现 | 三极管开关的设置限制（近地低端） | 大滤波电容，小滤波电容摆放限制= 小电容近芯片，大大电容近电池
  - **电路链路理解：** 包括：[电池端子连接-> 降压模块1584  ->单片机5v->单片机vcc->其他模块vcc（蜂鸣器，超声波，OLED，陀螺仪）]
                                        |->驱动VM      | ->驱动5v->步进电机
                                                       |->OpenMv 5v供电
                                                       |->循迹5v供电、三极管开关5v供电 
  ...
  -**原理图绘制:**
    --**模块封装自制：**
      -绘制要点:引脚顺序，绘制的引脚数量顺序正确对应
      -[检查：掏出实物一一对应确认引脚存在。][一些标注元素：vcc引脚小圈0.01inch标注；引脚。][引脚顺序和引脚方向正确：注意方向，放置网络标签的时候不容易粘连]
      -充分：没有缺少必要元件的原理图。降压模块、循迹模块、驱动模块、超声波、蜂鸣器、单片机、排针引出（不一定）、按键、led灯、oled显示屏
      -丰富：大多数模块都是排针排母进行占位。标记vcc引脚
    --**整体原理图：**
      -补全：补充被动元件（电阻、滤波电容、按键消抖电容、限流电阻、文字记录标注模块作用）
      -网络标签：1.首先确保单片机的网络标签正确，作为检查点。2.按照syscfg文件或者配置表设计完成网络标签的添加3.不使用的引脚利用非连接符占用
      -自查：对没使用过模块和方法，找视频和教程学一学，抄一抄，保险措施。[六引脚自锁开关：学习哪侧是公共引脚，按下时哪侧联通。] [三极管开关集电极-基电极-发射极，以及三极管的使用和驱动方式]
      -备用引脚：备用引脚的引出排针放一起，增加板子的通用以及可用性
      -网络标签验证：在网络标签都放置完成之后，可以删除模块的网络标签，删除单片机的网络标签，DRC检查原理图->网络标签是否错误添加
      -关键模块原理图：检查几个关键模块的原因他引脚，比如电机编码器的引脚顺序
      -原理图转PCB：通过原理图转换到PCB/选中一个模块的区域，右键布局传递
      
  -**PCB封装的绘制：**
    --信息具备：绘制模块封装必要信息：[整个模块的物理尺寸] [打孔的位置，孔位要思考孔半径1.8mm（m3）] [引脚序号从上往下对应原理图从上往下的顺序 1对应1 2对应2]
    --封装绘制顺序：1.放上对应间距焊盘，做好X轴Y轴对称/计算尺寸[快速对称方法为：对原点0，0相对偏移] 2.绘制略大的丝印，长宽手动输入，这样除2放置就对称 3.[放小圆点标注vcc区分，可以全选元素组合一下]
    --
