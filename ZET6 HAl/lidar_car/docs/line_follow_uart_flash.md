...
- PG13 启动任务一，PG9 启动任务二；运行中再次按 PG13 或 PG9 均急停并发送
  `MISSION_ABORT(0x84)`。PG12 在静止满 12 s 后低有效长按 2 s 时，MCU 直接清除
  本地人工校准并在三个连续车端时隙广播 `MAINTENANCE_RESET(0x85)`；不等待 Pi ACK。
  PG10 不参与任何任务或急停。
- 任务一离开 A 约 `130 mm/s`，任务二约 `150 mm/s`；雷达 B/C/D 坐标、B 后速度门和飞行阶段门见当前联调文档。
