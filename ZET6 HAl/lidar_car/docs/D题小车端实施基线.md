...
2. A 点全黑 `active_mask=0xFF` 只是位置标记。PG13 启动任务一，PG9 启动任务二；运行中再次按对应任务键均进入安全停车。PG10 不初始化、不读取，也不产生启动/停车事件。PG12 仅用于静止满 12 s 后长按 2 s 的人工维护复位：MCU 直接清除本地 Delta、校准标记和 CalibrationId，再广播 `0x85`，不等待 Pi ACK。
...
4. 灰度仍是主路径和最终 A 停车依据；JY901 累计偏航加编码器累计路径距离用于防止
   ...
   显示，不单独停车。
5. 失线进入 `LOST_HOLD` 后按最近四帧方向寻线，最多持续 12 s；连续三帧稳定重获后恢复循迹，超时才安全停车。陀螺仪/编码器短时异常只记录告警；赛题镜像保留 90 s 整圈看门狗，并增加 20 s 无编码器里程增量保护，分别记录 `lap_complete/lap_timeout` 与 `progress_timeout`。
...
- 当前物理维护事件为 PG12 低有效长按 2 s，且 `CAR_RUNNING=0` 连续 12 s；MCU 立即清除
...
因此 PG12 合格长按必须清除 MCU 本地校准并直接三帧广播 `0x85`。
