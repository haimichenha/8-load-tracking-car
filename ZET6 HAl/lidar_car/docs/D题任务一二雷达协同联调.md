...
> PG12 的工程标准是停车后短按。权威 V2.3 附录的“静止 12 s + 长按 2--3 s”仅作为
> 外部赛题条款和合规差距记录；当前固件没有对应的切换镜像，不能把两种触发条件合并。
...
| PG12 | 车辆停止后低有效短按，经 50 ms 消抖立即清除本地校准并广播三帧 `MAINTENANCE_RESET (0x85)`；三帧完成后可再次短按，不等待 Pi ACK |
| PG10 | 当前不参与启动、停车或任务控制 |

PG12 不改变 `0x85` 的地址、ResetId、Seq、payload 或三帧重传语义。当前单向 Pi
仍不具备 V2.3 的 Pi 清除/ACK 闭环；外部条款差异与逐项验证见
[D题_V23_联调与计分矩阵.md](D题_V23_联调与计分矩阵.md#pg12-维护复位标准)。
